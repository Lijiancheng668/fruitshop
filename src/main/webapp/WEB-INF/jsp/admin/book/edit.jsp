...
<div class="container" style="border: 1px solid #CCCCCC;">
    <form class="form-horizontal" role="form" id="bookForm" method="post" action="admin/book/update"
        ...
        <input type="hidden" name="fruitId" value="${bookInfo.fruitId}">
        <div class="form-group">
            <label for="name" class="col-sm-1 control-label">标题：</label>
            <div class="col-sm-5">
                <input type="text" class="form-control" id="name" name="name" value="${bookInfo.name}" placeholder="请输入水果名">
            </div>
            <span></span>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            <label for="place" class="col-sm-1 control-label">产地：</label>
            <div class="col-sm-5">
                <input type="text" class="form-control" id="place" name="place" value="${bookInfo.place}" placeholder="产地">
            </div>
            <span></span>
        </div>
            ...
            <div class="col-sm-5">
                <select name="bookCategoryId" id="bookCategoryId" class="form-control" style="width: 100px;">
                    <c:forEach items="${applicationScope.fruitCategories}" var="cate">
                        <option value="${cate.cateId}" ${cate.cateId == bookInfo.fruitCategoryId?"selected":""}>${cate.name}</option>
                    </c:forEach>
                </select>
            </div>
            <span></span>
        ...
        <div class="form-group">
            <label for="bookDesc" class="col-sm-1 control-label" style="padding: 0">水果详情：</label>
        </div>
        <!-- 加载编辑器的容器 -->
